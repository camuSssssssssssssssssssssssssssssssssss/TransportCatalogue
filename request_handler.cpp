...
const std::optional<graph::Router<double>::RouteInfo> RequestHandler::GetRoute(const std::string_view stop_from, const std::string_view stop_to) const{
...
void RequestHandler::ProcessRequests(const json::Node& stat_requests) const {
    ...
    for (auto& request : stat_requests.AsArray()) {
        ...
        if (type == "Route") {
            result.push_back(PrintRouting(request_map).AsMap());
        } 
    }
